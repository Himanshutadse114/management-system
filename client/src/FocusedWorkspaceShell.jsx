...
const WaiterWorkspace = lazy(() => import("./WaiterWorkspace"));
...
function ThemeToggle({ theme, onToggle }) {
  ...
  return (
    <button type="button" className="focused-theme" onClick={onToggle}>
      <Icon size={15} />
...
function ImpersonationBanner({ session, busy, error, onReturn }) {
  ...
  return (
    <div className="deva-impersonation-banner">
      <div className="deva-impersonation-copy">
        <ShieldCheck size={15} />
        <span>
          <strong>
            Working as{" "}
            {impersonation.role
              ? ROLE_LABELS[impersonation.role] || impersonation.role
              : "staff"}
          </strong>
          <small>{session?.user?.email} · Business Admin session</small>
        </span>
      </div>
      {error && <span className="deva-impersonation-error">{error}</span>}
      <button type="button" onClick={onReturn} disabled={busy}>
        <RotateCcw size={13} />
        {busy ? "Returning…" : "Return to Business Admin"}
      </button>
    </div>
  );
}
...
function scopedAccess(access, module) {
  if (module === MODULES.INVENTORY)
    return accessForBranchRoles(access, [
      "BRANCH_MANAGER",
      "INVENTORY_MANAGER",
    ]);
  if (module === MODULES.SALES)
    return accessForBranchRoles(access, ["BRANCH_MANAGER"]);
  if (module === MODULES.RESTAURANT_MANAGER)
    return accessForBranchRoles(access, ["BRANCH_MANAGER"]);
  if (module === MODULES.GROWTH)
    return accessForBranchRoles(access, ["BRANCH_MANAGER"]);
  if (module === MODULES.CONTROL)
    return accessForBranchRoles(access, ["BRANCH_MANAGER"]);
  if (module === MODULES.ECOSYSTEM)
    return accessForBranchRoles(access, ["BRANCH_MANAGER"]);
  if (module === MODULES.SETTINGS)
    return accessForBranchRoles(access, ["BRANCH_MANAGER"]);
  if (module === MODULES.CASHIER)
    return accessForBranchRoles(access, ["CASHIER"]);
  if (module === MODULES.WAITER)
    return accessForBranchRoles(access, ["WAITER"]);
  if ([MODULES.ANALYTICS, MODULES.REPORTS].includes(module))
    return accessForBranchRoles(access, ["BRANCH_MANAGER", "AUDITOR"]);
  return access;
}

function BranchOverview({ access, onOpen }) {
  const managers = (access?.branches || []).filter(
    (row) => row.role === "BRANCH_MANAGER",
  );
  const otherRoles = (access?.branches || []).filter(
    (row) => row.role !== "BRANCH_MANAGER",
  );
  const tools = [
    MODULES.INVENTORY,
  ...
  return (
    <div className="focused-overview">
      <section className="focused-overview-hero">
        <div>
          <div className="focused-kicker">Home</div>
          <h1>Your branches</h1>
          <p>
            Choose a branch job below. You only see the branches assigned to
            this staff account.
          </p>
        </div>
        <div className="focused-live">
          <ShieldCheck size={15} /> {managers.length} branch
          {managers.length === 1 ? "" : "es"}
        </div>
      </section>

      <div className="focused-branch-grid">
        {managers.map((row) => (
          <article key={row.membershipId}>
            <div className="focused-branch-icon">
              {row.branch?.type === "BAR_RESTAURANT" ? (
            ...
            <div>
              ...
              <small>
                {row.branch?.code} · {ROLE_LABELS[row.role]}
              </small>
            </div>
      ...
      <section className="focused-quick">
        <div className="focused-section-title">
          <div>
      ...
      {otherRoles.length > 0 && (
        <section className="focused-role-note">
          <UsersRound size={16} />
          <div>
            <strong>Other assigned duties</strong>
            <span>
              {otherRoles
                .map(
                  (row) =>
                    `${ROLE_LABELS[row.role] || row.role} · ${row.branch?.name || row.branchId}`,
                )
                .join("  •  ")}
            </span>
          </div>
...
function SimpleModuleIntro({ module }) {
  if (
    [MODULES.BRANCH_OVERVIEW, MODULES.WAITER, MODULES.CASHIER].includes(module)
  )
    return null;
  return (
...
function ModuleView({ module, token, access, onOpen }) {
  ...
  let content = null;
  if (module === MODULES.BRANCH_OVERVIEW)
    content = <BranchOverview access={access} onOpen={onOpen} />;
  if (module === MODULES.INVENTORY)
    content = <InventoryWorkspace token={token} access={narrowed} />;
  if (module === MODULES.SALES)
    content = <SalesWorkspace token={token} access={narrowed} />;
  if (module === MODULES.RESTAURANT_MANAGER)
    content = <RestaurantManagerWorkspace token={token} access={narrowed} />;
  if (module === MODULES.GROWTH)
    content = <GrowthWorkspace token={token} access={narrowed} />;
  if (module === MODULES.CONTROL)
    content = <OperationsWorkspace token={token} access={narrowed} />;
  if (module === MODULES.ECOSYSTEM)
    content = <EcosystemWorkspace token={token} access={narrowed} />;
  if (module === MODULES.CASHIER)
    content = <CashierWorkspace token={token} access={narrowed} />;
  if (module === MODULES.WAITER)
    content = <WaiterWorkspace token={token} access={narrowed} />;
  if (module === MODULES.ANALYTICS)
    content = <AnalyticsWorkspace token={token} access={narrowed} />;
  if (module === MODULES.REPORTS)
    content = <ReportsWorkspace token={token} access={narrowed} />;
  if (module === MODULES.SETTINGS)
    content = <SettingsWorkspace token={token} access={narrowed} />;
  return (
    <Suspense
      fallback={
    ...
    >
      <SimpleModuleIntro module={module} />
      {content}
    </Suspense>
  );
}
...
export default function FocusedWorkspaceShell() {
    ...
    try {
      ...
      setReturnError(
        error?.response?.data?.message ||
          error?.message ||
          "Could not return to the Business Admin account.",
      );
    } finally {
      setReturning(false);
    }
  ...
  if (!profile || !activeModule)
    return (
      <div className="focused-empty">
        <ShieldCheck size={24} />
        <strong>No work area assigned</strong>
        <span>Please ask your admin to assign your branch and job role.</span>
      </div>
    );

  const singlePurpose = profile.modules.length === 1;
  function open(module) {
    if (profile.modules.includes(module)) setActiveModule(module);
    setMobileOpen(false);
  }
  ...
  function Brand() {
    return (
      <button
        ...
        type="button"
        onClick={() => open(profile.defaultModule)}
      >
        <span className="focused-brand-mark">
          <Layers3 size={18} />
        </span>
        <span>
          <strong>Deva</strong>
  ...
  const mobileTabs = profile.modules
    .filter((module) => module !== MODULES.BRANCH_OVERVIEW)
    .slice(0, 4);
  return (
    <div className={`focused-shell scorm-theme-${theme}`}>
      ...
      <aside className="focused-sidebar">
        <Brand />
        <Nav />
        <div className="focused-sidebar-foot">
          <div>
            <ShieldCheck size={13} />
            <span>
              <strong>{profile.primaryRoleLabel}</strong>
              <small>{session?.user?.email}</small>
            </span>
          </div>
      ...
      {mobileOpen && (
        <div className="focused-mobile-overlay">
          <button
            className="focused-mobile-backdrop"
