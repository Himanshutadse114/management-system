...
function statusLabel(status) {
  ...
  return labels[String(status || '').toUpperCase()] || String(status || '').replaceAll('_', ' ').toLowerCase().replace(/(^|\s)\S/g, (letter) => letter.toUpperCase());
}
...
export default function WaiterWorkspace({ token, access }) {
  ...
  async function updateStatus(order, status) {
    try {
      ...
      await load();
      flash(status === 'SERVED' ? 'Order marked as served.' : 'Bill sent to cashier.');
    } catch (err) { setError(apiErrorMessage(err)); }
  }

  if (!assignments.length) {
    return <div className="waiter-page"><div className="waiter-empty"><UtensilsCrossed size={28}/><strong>No restaurant assigned</strong><span>Please ask your manager to assign this waiter account to a restaurant branch.</span></div></div>;
  }
  ...
  return <div className="waiter-page">
    <section className="waiter-head">
      <div><div className="waiter-kicker">Take Orders</div><h1>{branch?.name || 'Restaurant'}</h1><p>Choose a table, add items and place the order. After serving, send the bill to the cashier.</p></div>
      <div className="waiter-head-actions">
        {assignments.length > 1 && <select aria-label="Restaurant" value={membership?.membershipId || ''} onChange={(event) => setMembershipId(event.target.value)}>{assignments.map((row) => <option value={row.membershipId} key={row.membershipId}>{row.branch?.name} · {row.branch?.code}</option>)}</select>}
    ...
    <section className="waiter-section">
      <div className="waiter-section-head"><div><span>Step 1</span><h2>Choose a table</h2></div><small>{tables.length} tables</small></div>
      <div className="waiter-tables">{tables.map((table) => {
        ...
        return <button key={table.id} className={`waiter-table ${own ? 'own' : occupied ? 'occupied' : 'available'} ${selected ? 'selected' : ''}`} onClick={() => selectTable(table)} disabled={occupied}>
          <div className="waiter-table-icon"><Table2 size={19}/></div><strong>{table.name}</strong><span>{table.seats} seats</span><small>{own ? `${table.activeOrder.orderNumber} · ${statusLabel(table.activeOrder.status)}` : occupied ? 'Busy' : 'Available'}</small>
        </button>;
      })}</div>
    </section>
    ...
    <div className="waiter-order-layout">
      <section className="waiter-section waiter-menu-panel">
        <div className="waiter-section-head"><div><span>Step 2</span><h2>{selectedOrder ? `Add items to ${selectedOrder.table?.name || 'table'}` : selectedTable ? `Add items for ${selectedTable.name}` : 'Add items'}</h2></div><label className="waiter-search"><Search size={14}/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search food or drinks..."/></label></div>
      ...
      <aside className="waiter-section waiter-draft">
        <div className="waiter-section-head"><div><span>Your order</span><h2>{selectedOrder ? 'Add more items' : selectedTable ? selectedTable.name : 'New order'}</h2></div><ShoppingBag size={18}/></div>
        {!draft.length ? <div className="waiter-empty compact"><ShoppingBag size={22}/><strong>No items added</strong><span>Choose a table, then tap a price to add an item.</span></div> : <div className="waiter-draft-lines">{draft.map((row) => <div className="waiter-draft-line" key={row.priceOptionId}><div><strong>{row.productName}</strong><span>{row.priceLabel} · {formatMoney(row.priceMinor)}</span></div><div className="waiter-qty"><button aria-label="Reduce" onClick={() => quantity(row.priceOptionId, row.quantityUnits - 1)}><Minus size={11}/></button><span>{row.quantityUnits}</span><button aria-label="Add" onClick={() => quantity(row.priceOptionId, row.quantityUnits + 1)}><Plus size={11}/></button></div><strong>{formatMoney(BigInt(row.priceMinor || 0) * BigInt(row.quantityUnits))}</strong></div>)}</div>}
        <div className="waiter-total"><span>Total</span><strong>{formatMoney(draftTotal)}</strong></div>
        <button className="waiter-primary" onClick={submitOrder} disabled={busy || !draft.length || (!tableId && !targetOrderId)}>{targetOrderId ? 'Add items to order' : 'Place order'}<ChevronRight size={15}/></button>
      </aside>
    </div>

    <section className="waiter-section">
      <div className="waiter-section-head"><div><span>Step 3</span><h2>Serve & send bill</h2></div><small>{activeOrders.length} active</small></div>
      {!activeOrders.length ? <div className="waiter-empty compact"><CheckCircle2 size={22}/><strong>No open orders</strong><span>Orders you place will appear here.</span></div> : <div className="waiter-active-orders">{activeOrders.map((order) => <article key={order.id} className={`waiter-order status-${String(order.status).toLowerCase()}`}>
        <div className="waiter-order-top"><div><span>{statusLabel(order.status)}</span><strong>{order.table?.name || order.orderNumber}</strong><small>{order.orderNumber} · <Clock3 size={10}/> {new Date(order.createdAt).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})}</small></div><strong>{formatMoney(order.totalMinor)}</strong></div>
        ...
        <div className="waiter-order-actions"><button onClick={() => selectOwnOrder(order)}>Add items</button>{order.status === 'OPEN' && <button onClick={() => updateStatus(order, 'SERVED')}>Mark served</button>}{['OPEN','SERVED'].includes(order.status) && <button className="bill" onClick={() => updateStatus(order, 'AWAITING_PAYMENT')}>Send bill to cashier</button>}{order.status === 'AWAITING_PAYMENT' && <span className="waiter-awaiting"><Clock3 size={12}/>Waiting for payment</span>}</div>
      </article>)}</div>}
    </section>
  </div>;
}