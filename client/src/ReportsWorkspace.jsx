...
export default function ReportsWorkspace({ token, access }) {
  ...
  if(!hasTenantView&&!branchMemberships.length)return <div className="reports-page"><div className="reports-empty"><FileText size={25}/><strong>Reports access is not assigned</strong><span>Tenant Admins, Auditors and Branch Managers can generate management reports.</span></div></div>;

  return <div className="reports-page">
    <section className="reports-hero"><div><div className="reports-mini">PDF · Excel · multilingual · archived</div><h2>Reports</h2><p>Generate detailed operational reports directly from source-of-truth sales, inventory, restaurant and finance records.</p></div><button className="scorm-button-secondary" onClick={loadHistory}><RefreshCw size={14}/>Refresh history</button></section>
    ...
    <div className="reports-layout">
      <form className="reports-panel report-builder" onSubmit={generate}>
        <div className="reports-panel-head"><div><div className="reports-mini">Report builder</div><h3>Generate report</h3></div><FileText size={18}/></div>
        ...
        <label><span>Scope</span><select value={branchId} onChange={(e)=>setBranchId(e.target.value)}>{canUseTenantScope&&<option value="">All branches / consolidated</option>}{branches.map((row)=><option key={row.id} value={row.id}>{row.name} · {row.code}</option>)}</select></label>
        <label><span>Report</span><select value={form.reportType} onChange={(e)=>setForm({...form,reportType:e.target.value})}>{reportCards.map((row)=><option key={row.id} value={row.id}>{row.name}</option>)}</select></label>
        {selected&&<div className="report-description">{selected.description}</div>}
        <div className="report-pair"><label><span>From</span><input type="date" value={form.from} onChange={(e)=>setForm({...form,from:e.target.value})}/></label><label><span>To</span><input type="date" value={form.to} onChange={(e)=>setForm({...form,to:e.target.value})}/></label></div>
        <div className="report-pair"><label><span>Format</span><select value={form.format} onChange={(e)=>setForm({...form,format:e.target.value})}>{(catalog.formats||[]).map((value)=><option key={value}>{value}</option>)}</select></label><label><span>Language</span><select value={form.locale} onChange={(e)=>setForm({...form,locale:e.target.value})}>{(catalog.locales||[]).map((row)=><option key={row.id} value={row.id}>{row.name}</option>)}</select></label></div>
        <div className="report-language-note"><Languages size={15}/><span>PDF uses Noto + Indic shaping for Hindi/Marathi. XLSX keeps multilingual headings editable in Excel.</span></div>
        <button className="scorm-button-primary report-generate" disabled={busy||!tenantId}><FileText size={14}/>{busy?'Generating…':'Generate report'}</button>
      </form>

      <section className="reports-panel report-catalogue"><div className="reports-panel-head"><div><div className="reports-mini">Available outputs</div><h3>Report catalogue</h3></div><FileSpreadsheet size={18}/></div><div className="report-card-grid">{reportCards.map((row)=><button type="button" key={row.id} className={form.reportType===row.id?'report-card active':'report-card'} onClick={()=>setForm({...form,reportType:row.id})}><div>{row.id==='INVENTORY_VALUATION'||row.id==='STOCK_MOVEMENTS'?<Store size={16}/>:<FileText size={16}/>}</div><strong>{row.name}</strong><span>{row.description}</span></button>)}</div></section>
    </div>

    <section className="reports-panel history-panel"><div className="reports-panel-head"><div><div className="reports-mini">Stored securely</div><h3>Report history</h3></div><span className="reports-count">{history.length}</span></div>{!history.length?<div className="reports-empty compact"><FileText size={22}/><strong>No reports generated yet</strong><span>Generated files will appear here and remain available through authenticated download.</span></div>:<div className="report-history"><div className="report-history-head"><span>Report</span><span>Scope</span><span>Range</span><span>Language</span><span>Status</span><span></span></div>{history.map((report)=><article className="report-history-row" key={report.id}><div className="report-file-icon">{report.format==='XLSX'?<FileSpreadsheet size={17}/>:<FileText size={17}/>}</div><div className="report-history-main"><strong>{reportName(catalog.reports,report.reportType)}</strong><span>{report.fileName||`${report.format} report`} · {bytes(report.sizeBytes)}</span></div><span>{report.branchId?branches.find((b)=>b.id===report.branchId)?.name||'Branch':'All branches'}</span><span>{report.rangeFrom} → {report.rangeTo}</span><span className="report-locale"><Languages size={11}/>{report.locale==='hi'?'हिन्दी':report.locale==='mr'?'मराठी':'English'}</span><span className={`report-status ${report.status.toLowerCase()}`}>{report.status==='READY'&&<CheckCircle2 size={11}/>} {report.status}</span><button className="report-download" onClick={()=>download(report)} disabled={report.status!=='READY'||downloading===report.id}><Download size={13}/>{downloading===report.id?'Downloading…':'Download'}</button></article>)}</div>}</section>
  </div>;
}
