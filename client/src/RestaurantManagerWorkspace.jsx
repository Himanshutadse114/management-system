...
function ScopeSelector({ token, access, scope, setScope, setBranch }) {
  ...
  return <div className="restaurant-scope"><label><span>Business</span><select value={scope.tenantId} onChange={(event) => setScope({ tenantId: event.target.value, branchId: '' })}>{!tenants.length && <option value="">No business</option>}{tenants.map((row) => <option value={row.id} key={row.id}>{row.name}</option>)}</select></label><label><span>Restaurant outlet</span><select value={scope.branchId} onChange={(event) => { const id = event.target.value; setScope({ ...scope, branchId: id }); setBranch(branches.find((row) => row.id === id) || null); }}>{!branches.length && <option value="">No restaurant outlet</option>}{branches.map((row) => <option value={row.id} key={row.id}>{row.name} · {row.code}</option>)}</select></label></div>;
}
...
export default function RestaurantManagerWorkspace({ token, access }) {
  ...
  async function changeStatus(order, status) {
    try { setError(''); await api.post(`${base}/orders/${order.id}/status`, { status }, { headers: authHeaders(token) }); await loadAll(); flash(`Order marked ${status.toLowerCase().replaceAll('_',' ')}.`); }
    catch (err) { setError(apiErrorMessage(err)); }
  }
  async function pay(order) {
    try { setError(''); await api.post(`${base}/orders/${order.id}/pay`, { paymentMethod }, { headers: authHeaders(token) }); await loadAll(); flash(`Payment recorded for ${order.orderNumber}.`); }
    catch (err) { setError(apiErrorMessage(err)); }
  }
  async function confirmCancel() {
    if (!cancelTarget || !cancelReason.trim()) return;
    try { setBusy(true); setError(''); await api.post(`${base}/orders/${cancelTarget.id}/cancel`, { reason: cancelReason.trim() }, { headers: authHeaders(token) }); setCancelTarget(null); setCancelReason(''); await loadAll(); flash('Order cancelled and stock restored.'); }
    catch (err) { setError(apiErrorMessage(err)); }
    finally { setBusy(false); }
  }
  async function createTable(event) {
    event.preventDefault();
    try { setError(''); await api.post(`${base}/tables`, { ...tableForm, seats: Number(tableForm.seats || 4) }, { headers: authHeaders(token) }); setTableForm({ name:'', code:'', seats:'4' }); await loadAll(); flash('Table and public-menu QR created.'); }
    catch (err) { setError(apiErrorMessage(err)); }
  }
  async function publishMenu(event) {
    ...
    try {
      ...
      await loadAll(); flash('Menu item published.');
    } catch (err) { setError(err.message || apiErrorMessage(err)); }
  }
  ...
  async function copyLink(table) {
    ...
    catch (_) { flash('Copy failed. Open the menu and copy the browser URL.'); }
  }

  if (!scope.branchId) return <div className="restaurant-page"><div className="restaurant-hero"><div><div className="restaurant-mini">Manager workspace</div><h2>Restaurant Management</h2><p>Select one of your Bar + Restaurant outlets.</p></div></div><ScopeSelector token={token} access={access} scope={scope} setScope={setScope} setBranch={setBranch}/><Empty icon={UtensilsCrossed} title="No restaurant selected" body="Only restaurant outlets within your assigned management scope appear here."/></div>;

  return <div className="restaurant-page">
    <div className="restaurant-hero"><div><div className="restaurant-mini">Branch manager · table service · menu controls</div><h2>{branch?.name || 'Restaurant Management'}</h2><p>Manage tables, public menus and service exceptions. Waiters use their own restricted table-service workspace.</p></div><button className="scorm-button-secondary" onClick={loadAll} disabled={busy}><RefreshCw size={14} className={busy?'spin':''}/>Refresh</button></div>
