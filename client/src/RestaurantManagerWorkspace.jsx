...
export default function RestaurantManagerWorkspace({ token, access }) {
  ...
  return <div className="restaurant-page">
    ...
    <div className="workspace-tabs">{[{label:'Orders',icon:ClipboardList},{label:'Tables',icon:QrCode},{label:'Menu',icon:UtensilsCrossed}].map(({label,icon:Icon}) => <button key={label} className={tab===label?'is-active':''} onClick={() => setTab(label)}><Icon size={15}/>{label}</button>)}</div>
