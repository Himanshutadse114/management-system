      ...
      <div className="sa-shell">
        ...
        <main className="sa-card auth-enter">
          <section className="sa-brand-panel">
            ...
            <p className="sa-copy">One workspace for multi-outlet inventory, bar and restaurant operations, wine-shop sales, staff accountability and management reporting.</p>
            ...
            <div className="sa-notice">
              ...
              <div>Google verifies identity first. The backend then resolves the current tenant, branch and role before protected data is returned.</div>
            </div>
          </section>
          ...
          <section className="sa-form-panel">
            ...
            <p className="sa-form-sub">Continue with the Google account assigned by your platform or tenant administrator.</p>
...
function SectionHeader({ eyebrow, title, count, icon: Icon }) {
  return (
    <div className="scorm-panel-header ops-panel-header">
      <div><div className="scorm-eyebrow">{eyebrow}</div><h3>{title}</h3></div>
    ...
    <section className="scorm-panel ops-management-panel">
      ...
      <div className="ops-admin-grid">
        <form className="ops-form" onSubmit={createTenant}>
          <div className="ops-form-heading"><div className="scorm-action-icon"><Plus size={17} /></div><div><div className="scorm-eyebrow">Super Admin</div><h4>Create tenant</h4></div></div>
        ...
        <div className="ops-list">
          ...
          {!tenants.length && <div className="ops-empty"><div className="scorm-empty-icon"><Building2 size={18} /></div><strong>No tenants yet</strong><span>Create the first business group to begin outlet setup.</span></div>}
          {tenants.map((tenant) => <div className="ops-row" key={tenant.id}><div className="ops-entity"><div className="ops-avatar">{tenant.name.slice(0, 2).toUpperCase()}</div><div><strong>{tenant.name}</strong><span>{tenant.slug}</span></div></div><span className={`ops-status ${tenant.status === 'ACTIVE' ? 'is-active' : ''}`}>{tenant.status}</span></div>)}
        </div>
      </div>
    </section>
  );
...
function TenantBranches({ token, membership }) {
  ...
  async function createBranch(event) {
    ...
    try {
      ...
      setForm({ name: '', code: '', type: 'BAR_RESTAURANT' }); await load();
    } catch (err) { setError(apiErrorMessage(err)); }
    finally { setBusy(false); }
  }
    ...
    <section className="scorm-panel ops-management-panel">
      ...
      <div className="ops-admin-grid">
        <form className="ops-form" onSubmit={createBranch}>
          <div className="ops-form-heading"><div className="scorm-action-icon"><Plus size={17} /></div><div><div className="scorm-eyebrow">Tenant Admin</div><h4>Add outlet</h4></div></div>
        ...
        <div className="ops-list">
          ...
          {!branches.length && <div className="ops-empty"><div className="scorm-empty-icon"><Store size={18} /></div><strong>No outlets configured</strong><span>Create the first branch to begin inventory and staff setup.</span></div>}
          {branches.map((branch) => <div className="ops-row" key={branch.id}><div className="ops-entity"><div className="ops-avatar">{branch.type === 'BAR_RESTAURANT' ? <Wine size={16} /> : <Store size={16} />}</div><div><strong>{branch.name}</strong><span>{branch.code} · {branch.type === 'BAR_RESTAURANT' ? 'Bar + Restaurant' : 'Wine Shop'}</span></div></div><span className="ops-status is-active">{branch.status}</span></div>)}
        </div>
      </div>
    </section>
  );
...
function Overview({ token, access, isSuperAdmin, tenantAdmin, primaryRole, onOpenSection }) {
  ...
  return (
    <div className="platform-page">
      <section className="scorm-page-hero"><div className="page-hero-row"><div className="page-hero-copy"><div className="hero-meta-row"><span className="scorm-eyebrow">Operate · control · reconcile · report</span><span className="scorm-health-pill is-online"><span className="scorm-health-dot" /> Operations online</span></div><h2 className="scorm-display"><span>Operations</span> <span className="wb-accent">Workbench</span></h2><p>Manage outlets, staff, inventory, sales, restaurant service, analytics and multilingual reports from one branch-aware operating view.</p></div><div className="hero-actions"><button className="scorm-button-secondary" onClick={() => onOpenSection('Reports')}><FileText size={15} /> Reports</button><button className="scorm-button-primary" onClick={() => onOpenSection(isSuperAdmin ? 'Tenants' : 'Branches')}><Plus size={15} /> {isSuperAdmin ? 'Create tenant' : 'Add outlet'}</button></div></div></section>
      <div className="metric-grid">{stats.map((stat) => <MetricCard key={stat.label} {...stat} />)}</div>
      <div className="overview-grid">
        <div>{isSuperAdmin && <PlatformTenants token={token} />}{!isSuperAdmin && tenantAdmin && <TenantBranches token={token} membership={tenantAdmin} />}{!isSuperAdmin && !tenantAdmin && <RoadmapPanel />}</div>
        <div className="overview-side">
          <section className="scorm-progress-hero"><div className="progress-hero-top"><div><div className="scorm-progress-kicker">System foundation</div><h3>Operations readiness</h3></div><div className="scorm-progress-icon"><Activity size={18} /></div></div><div><div className="scorm-progress-number">Phase 6</div><div className="progress-copy">Inventory, POS, restaurant operations, analytics and staff access are connected. Detailed PDF/XLSX reports now support English, Hindi and Marathi with secure report history and downloads.</div><div className="scorm-progress-track is-dark"><div className="scorm-progress-fill" style={{ width: '88%' }} /></div></div></section>
          <section className="scorm-panel quick-panel"><SectionHeader eyebrow="Workspace" title="Quick access" /><div className="quick-list"><button onClick={() => onOpenSection('Inventory')}><PackageSearch size={15} /><span><strong>Inventory</strong><small>ML stock and purchases</small></span><ChevronRight size={14} /></button><button onClick={() => onOpenSection('Restaurant')}><UtensilsCrossed size={15} /><span><strong>Restaurant</strong><small>Tables, QR menu and waiter service</small></span><ChevronRight size={14} /></button><button onClick={() => onOpenSection('Analytics')}><BarChart3 size={15} /><span><strong>Analytics & P&amp;L</strong><small>Revenue, margin and controls</small></span><ChevronRight size={14} /></button><button onClick={() => onOpenSection('Reports')}><FileText size={15} /><span><strong>Reports</strong><small>PDF, Excel and multilingual</small></span><ChevronRight size={14} /></button></div></section>
        </div>
      </div>
      <div className="action-grid"><button className="scorm-action-card" onClick={() => onOpenSection('Restaurant')}><div className="scorm-action-icon"><UtensilsCrossed size={17} /></div><div className="action-title">Restaurant</div><div className="action-copy">Operate tables, waiter orders, public QR menus and unresolved-order reconciliation.</div><div className="scorm-action-arrow"><ChevronRight size={15} /></div></button><button className="scorm-action-card" onClick={() => onOpenSection('Analytics')}><div className="scorm-action-icon"><BarChart3 size={17} /></div><div className="action-title">Analytics & P&amp;L</div><div className="action-copy">Compare branches, analyse product mix, margins, expenses, stock risk and unresolved orders.</div><div className="scorm-action-arrow"><ChevronRight size={15} /></div></button><button className="scorm-action-card" onClick={() => onOpenSection('Reports')}><div className="scorm-action-icon"><FileText size={17} /></div><div className="action-title">Reports</div><div className="action-copy">Generate detailed PDF and Excel reports in English, Hindi or Marathi and download them securely.</div><div className="scorm-action-arrow"><ChevronRight size={15} /></div></button></div>
    </div>
  );
}
...
function Dashboard() {
  ...
  const groups = useMemo(() => {
    const platform = [{ label: 'Overview', icon: BarChart3 }, ...(isSuperAdmin ? [{ label: 'Tenants', icon: Building2 }] : []), { label: 'Branches', icon: Store }];
    return [
  ...
  function Navigation() {
    return <nav className="scorm-nav">{groups.map((group, index) => <div className={index ? 'nav-group nav-group-spaced' : 'nav-group'} key={group.label}><div className="scorm-nav-section">{group.label}</div><div className="nav-items">{group.items.map(({ label, icon: Icon }) => { const active = activeSection === label; return <button key={label} type="button" className={`scorm-nav-item ${active ? 'scorm-nav-active' : ''}`} onClick={() => openSection(label)}><span className="scorm-nav-icon"><Icon size={16} strokeWidth={active ? 2.2 : 1.9} /></span><span>{label}</span>{active && <ChevronRight size={14} className="scorm-nav-chevron" />}</button>; })}</div></div>)}</nav>;
  }

  function Brand() {
    return <button type="button" className="scorm-brand" onClick={() => openSection('Overview')}><span className="scorm-brand-mark"><Layers3 size={19} /></span><span className="brand-copy"><strong className="scorm-brand-name">OUTLET <em>OS</em></strong><small>Hospitality operations platform</small></span></button>;
  }
    ...
    <div className={`scorm-editorial scorm-theme-${theme}`}>
      ...
      <div className="scorm-shell-content">
        ...
        <main className="scorm-main">
          ...
          {activeSection === 'Branches' && !tenantAdmin && <div className="platform-page standalone-page"><RoadmapPanel /></div>}
          ...
          {activeSection === 'Analytics' && <AnalyticsWorkspace token={token} access={access} />}
          {activeSection === 'Reports' && <ReportsWorkspace token={token} access={access} />}
      ...
      ].map(({ label, short, icon: Icon }) => <button key={label} className={`scorm-mobile-tab ${activeSection === label ? 'is-active' : ''}`} onClick={() => openSection(label)}><Icon size={17} /><span>{short || label}</span></button>)}</div>
    </div>
  );
}
