...
router.get('/:tenantId', loadTenant, requireTenantAdmin, async (req, res) => {
...
router.get('/:tenantId/branches', loadTenant, requireTenantAdmin, async (req, res) => {
...
router.post('/:tenantId/branches', loadTenant, requireTenantAdmin, async (req, res, next) => {
  try {
    ...
    if (!BRANCH_TYPES.includes(type)) {
      return res.status(400).json({ message: `Branch type must be one of: ${BRANCH_TYPES.join(', ')}` });
    }
    ...
    if (error?.name === 'SequelizeUniqueConstraintError') {
      return res.status(409).json({ message: 'This branch code already exists for the tenant.' });
    }
    next(error);
  }
});
...
router.post('/:tenantId/branches/:branchId/members', loadTenant, requireTenantAdmin, async (req, res, next) => {
  try {
    ...
    if (!BRANCH_ROLES.includes(role)) {
      return res.status(400).json({ message: `Role must be one of: ${BRANCH_ROLES.join(', ')}` });
    }
